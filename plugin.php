<?php
    final class smb
    {
        ...
        public function __construct($version, $base)
        {
            ...
            $this->db = smb_db::GetInstance();
            register_activation_hook($base, array($this, 'initialize'));
            if (get_option('smb_db_version') != $this->db->db_version) {
                $this->initialize();
            }
            add_action('init', array($this, 'smb_load_textdomain' ));
            add_action('admin_menu', array($this, 'add_menu_items'));
            add_action('admin_enqueue_scripts', array($this, 'backend_enqueue'));
            add_action('wp_enqueue_scripts', array($this, 'frontend_enqueue'));
            add_filter('plugin_row_meta', array($this, 'plugin_row_meta' ), 10, 2);
            add_action('wp_ajax_smbAjax', array($this, 'smbAjax'));
            add_shortcode('smb-share-buttons', array($this, 'frontend_share'));
            add_shortcode('smb-follow-buttons', array($this, 'frontend_follow'));
            $this->setPositionSettingFilters();
            $this->pluginNameComment = '<!-- Social Share -->';
            $this->localizeStrings();
        }

        public function smb_load_textdomain()
        {
            load_plugin_textdomain('smb', false, dirname($this->plugin_base) . '/languages');
        }

        private function localizeStrings()
        {
           
            $this->defaultShareText = esc_html__("Share on Social Media", 'smb');
        }

        public function plugin_row_meta($row_meta, $file)
        {
            if ($this->plugin_base == $file) {
                $row_meta[] = '<a href="'.admin_url('admin.php?page=smb').'" aria-label="Options">Options</a>';
            }
            return $row_meta;
        }
        ...
        {
            $user_caps = apply_filters('smb_user_capabilities', $this->user_caps);
            $this->page_hook_s = add_menu_page('Social Share', ' Social Share', $user_caps, 'smb', array($this, 'backend_share'), $this->base_url . "assets/img/icon.png");
            add_submenu_page('smb', 'Social Share', 'Social Share', $user_caps, 'smb');
        ...
        public function backend_enqueue($hook)
        {
            if ($hook == $this->page_hook_s || $hook == $this->page_hook_f) {
                wp_enqueue_style('smb-backend', $this->base_url . '/assets/css/backend.css', false, $this->version, 'all');
                $this->frontend_enqueue();
                wp_enqueue_script('smb-tablednd', $this->base_url . '/js/jquery.tablednd.js');
                wp_enqueue_script('smb-script', $this->base_url . '/js/backend.js', array('jquery', 'wp-color-picker'), $this->version);
                wp_enqueue_style('wp-color-picker');
                wp_localize_script('smb-script', 'msgs', array(
                'alreadySaved'  => esc_html__("Current settings already saved..", 'smb'),
                'error' => esc_html__("Couldn't save settings..", 'smb'),
                'fieldErrorMultiple' => esc_html__("fields are not valid profile links..", 'smb'),
                'fieldErrorSingle' => esc_html__("field is not a valid profile link..", 'smb'),
                'fieldErrorExpected' => esc_html__(" Expected https://(site).com/(profile) or similar.", 'smb'),
            ));
            }
        }
        ...
        {
            wp_enqueue_style('smb-stylesheet', $this->base_url . '/assets/css/frontend.css', false, $this->version, 'all');
            wp_enqueue_style('smb-lato-font', 'https://fonts.googleapis.com/css?family=Lato:300,400,700&display=swap', false, $this->version, 'all');
        }
        ...
        public function frontend_share()
        {
            ...
            if ($buttons) {
                ...
                $thumb = isset($thumb) && !empty($thumb) ? $thumb : wp_get_attachment_image_src(get_theme_mod('custom_logo'))[0];
                ...
                ob_start();
                echo '<div class="smb-sharebuttons-output-wrapper">';
                echo $this->pluginNameComment;
                echo '<div class="smb-output-textstring">'.esc_attr($this->defaultShareText).'</div>';
                echo '<div class="smb-button-wrapper-flat">';
                foreach ($buttons as &$button) {
                    $link = str_replace(
                        array('{url}','{title}','{img}','{description}'),
                        array($permlink,$title,$thumb,$excerpt),
                        $button['share']
                    );
                    echo '<span class="smb-share-'.esc_attr($button['class']).'"><a href="'.esc_url($link).'" rel="nofollow" target="_blank">'.$button['icon'];
        ...
        public function frontend_follow()
        {
            ...
            if ($buttons) {
                ob_start();
                echo '<div class="smb-followbuttons-output-wrapper">';
                echo $this->pluginNameComment;
                echo '<div class="smb-output-textstring">'.esc_attr($this->defaultFollowText).'</div>';
                echo '<div class="smb-button-wrapper-flat">';
                foreach ($buttons as &$button) {
                    if ($button['class']=='email') {
                        continue;
                    }
                    echo '<span class="smb-follow-'.esc_attr($button['class']).'"><a href="'.esc_url($button['follow_url']).'" rel="nofollow" target="_blank">'.$button['icon'];
                    echo esc_attr($button['class']);
                    echo '</a></span>';
                }
                echo '</div></div>';
        ...
        public function addToContent($content)
        {
            ...
            $shareSettings['pages_addAtStart']==1||$shareSettings['pages_addAtEnd']==1) {
                $scShare = do_shortcode('[smb-share-buttons]');
            }
            ...
            $followSettings['pages_addAtStart']==1||$followSettings['pages_addAtEnd']==1) {
                $scFollow = do_shortcode('[smb-follow-buttons]');
            }
        ...
        public function addSidebar()
        {
            ...
            if ($settings) {
                if ($settings[0]['sidebar'] == $settings[1]['sidebar']) {
                    $hide = $settings[0]['hide'] == 1 || $settings[1]['hide'] == 1 ? ' smb-hideonmobile' : '';
                    $class = 'smb-sidebar-wrapper';
                    ...
                    echo '<div class="smb-sidebar-wrapper '.$class.$hide.'">';
                    echo do_shortcode('[smb-share-buttons]');
                    echo do_shortcode('[smb-follow-buttons]');
                    echo '</div>';
                } else {
                    if ($settings[0]['sidebar'] > 0) {
                        $hide = $settings[0]['hide'] == 1 ? ' smb-hideonmobile' : '';
                        $class = 'smb-sidebar-wrapper';
                        if ($settings[0]['sidebar']==1) {
                            $class.='-leftcenter';
                        }
                    ...
                    if ($settings[1]['sidebar'] > 0) {
                        $hide = $settings[1]['hide'] == 1 ? ' smb-hideonmobile' : '';
                        $class = 'smb-sidebar-wrapper';
