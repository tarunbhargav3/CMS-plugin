  ...
  $(document).ready(function() {
    ...
    $("table").each(function() {
      if ($(this).data("table") == "dnd") {
        $(this).tableDnD({
          onDragStyle:
            ($(this).data("ondragstyle") &&
              parseStyle($(this).data("ondragstyle"))) ||
            null,
          onDropStyle:
            ($(this).data("ondropstyle") &&
              parseStyle($(this).data("ondropstyle"))) ||
            null,
          onDragClass:
            ($(this).data("ondragclass") == undefined && "tDnD_whileDrag") ||
            $(this).data("ondragclass"),
          onDrop:
            $(this).data("ondrop") &&
            new Function("table", "row", $(this).data("ondrop")), // 'return eval("'+$(this).data('ondrop')+'");') || null,
          onDragStart:
            $(this).data("ondragstart") &&
            new Function("table", "row", $(this).data("ondragstart")), // 'return eval("'+$(this).data('ondragstart')+'");') || null,
          scrollAmount: $(this).data("scrollamount") || 5,
          sensitivity: $(this).data("sensitivity") || 10,
          hierarchyLevel: $(this).data("hierarchylevel") || 0,
          indentArtifact:
            $(this).data("indentartifact") ||
  ...
  jQuery.tableDnD = {
    ...
    build: function(options) {
      ...
      this.each(function() {
        ...
        this.tableDnDConfig.hierarchyLevel && $.tableDnD.makeIndented(this);
      });
      ...
      (config.dragHandle &&
        ...
        $(table.rows).each(function() {
          ...
          if (!$(this).hasClass("nodrag")) {
            $(this).bind(startEvent, function(e) {
              if (e.target.className == "smb_dragRow") {
    ...
    findDropTargetRow: function(draggedRow, y) {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        if (y > rowY - rowHeight && y < rowY + rowHeight)
          if (
            draggedRow.is(row) ||
            (config.onAllowDrop && !config.onAllowDrop(draggedRow, row)) ||
    ...
    processMouseup: function() {
      ...
      if (!this.currentTable || !droppedRow) return null;
    ...
    tableData: function(table) {
      ...
      if (!table || !table.id || !table.rows || !table.rows.length)
        return {
          error: {
            code: 500,
            message: "Not a valid table, no serializable unique id provided."
          }
        };

      rows =
        (config.autoCleanRelations && table.rows) || $.makeArray(table.rows);
